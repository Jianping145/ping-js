/**
 * rou.js - 肉視頻 T4/蜂蜜（内嵌 zlib + play 内解包 m3u8）
 * 配置: { "key":"rou", "name":"肉視頻", "type":3, "api":"https://.../rou.js" }
 */
...
function _atob(s) {
    if (typeof atob === 'function') { try { return atob(s) } catch (e) {} }
...
function _btoa(s) {
    if (typeof btoa === 'function') { try { return btoa(s) } catch (e) {} }
    const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
    let out = ''
    for (let i = 0; i < s.length; i += 3) {
        ...
        out += (i + 1 < s.length ? chars[(n >> 6) & 63] : '=') + (i + 2 < s.length ? chars[n & 63] : '=')
    }
    return out
}
...
function unpackRouPng(bytes) {
    ...
    while (n + 12 <= bytes.length) {
        ...
        if (typ === 'roUd') {
            ...
            console.log('roUd flag=' + flag + ' raw=' + payload.length)
            if (flag & 1) {
                payload = tryInflate(payload)
                console.log('inflated=' + (payload ? payload.length : -1))
            }
            return payload
        }
        n = dataStart + ln + 4
...
function tryInflate(data) {
    try {
        const p = (typeof __pako !== 'undefined' && __pako) || (typeof globalThis !== 'undefined' && globalThis.__pako) || null
        if (p && p.inflate) {
            const out = p.inflate(data)
            return out instanceof Uint8Array ? out : new Uint8Array(out)
        }
    } catch (e) { console.log('inflate err: ' + e) }
    console.log('no zlib')
    return data
}
...
function rewriteM3u8(content, base) {
    ...
    for (const line of lines) {
        ...
        if (s && s[0] !== '#') {
            ...
            out.push(line.replace(/URI="([^"]+)"/g, (_, u) => 'URI="' + toProxy(joinUrl(base, u)) + '"'))
        } else {
            out.push(line)
        }
    }
...
async function init(cfg) {
    try {
        if (cfg) {
            SITE_KEY = cfg.skey || cfg.key || cfg.siteKey || SITE_KEY
        }
    } catch (e) {}
    console.log('rou init key=' + SITE_KEY + ' pako=' + (typeof __pako !== 'undefined'))
...
async function detail(ids) {
    try {
        ...
        id = id.replace(/\/$/, '')
        const html = await fetchHtml(HOST + '/v/' + id)
        let title = id, pic = ''
...
async function proxy(params) {
    try {
        ...
        if (raw.indexOf('http') !== 0 && raw.indexOf('/') !== 0) {
            try {
                const pad = '===='.slice(0, (4 - (raw.length % 4)) % 4)
                const dec = _atob(raw.replace(/-/g, '+').replace(/_/g, '/') + pad)
                if (dec.indexOf('http') === 0) raw = dec
            } catch (e) {}
        }
...
export function __jsEvalReturn() {
    return {
        init, home, homeVod, category, detail, play, search, proxy,
    }
}
