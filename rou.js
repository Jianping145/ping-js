...
function getBuf(res) {
    ...
    // buffer=1 有时返回 base64
    if (res && res.buffer && typeof res.buffer === 'string') {
        try {
            return b64ToBytes(res.buffer)
        } catch (e) {}
    }
...
async function fetchBin(url) {
    ...
    for (const opt of tries) {
        try {
            ...
            const t = parseRes(res)
            if (typeof t === 'string' && t.length > 8) {
                const a = new Uint8Array(t.length)
                for (let i = 0; i < t.length; i++) a[i] = t.charCodeAt(i) & 0xff
                if (a[0] === 0x89 || a[0] === 0x47 || t.indexOf('#EXT') === 0) return a
            }
        } catch (e) {
            console.log('fetchBin try fail: ' + e)
        }
    }
    return null
}
...
function parseList(html) {
    ...
    while ((m = re.exec(html)) !== null) {
        const path = m[1]
        const vid = path.replace(/^\/v\//, '').replace(/\/$/, '')
        if (!vid || seen[vid]) continue
        seen[vid] = 1
        const slice = html.slice(m.index, m.index + 1500)
        let title = vid
        const alt = slice.match(/alt="([^"]{2,120})"/)
        if (alt && alt[1] && alt[1].trim()) title = alt[1].trim()
        let pic = ''
        const imgs = slice.match(/src="(https?:\/\/[^"]+)"/g) || []
        for (const im of imgs) {
            const u = im.replace(/^src="/, '').replace(/"$/, '')
            if (/cover|rn252|jpg|jpeg|webp/i.test(u)) {
                pic = u
                break
            }
        }
        if (!pic && imgs.length) pic = imgs[imgs.length - 1].replace(/^src="/, '').replace(/"$/, '')
        let remarks = ''
        const resMark = slice.match(/>(\d{3,4})<!--\s*-->P</) || slice.match(/>(\d{3,4})P</)
        if (resMark) remarks = resMark[1] + 'P'
...
function extractEvPlayUrl(html) {
    ...
    try {
        ...
        console.log('ev decode fail: ' + e)
        return ''
    }
}

function b64ToBytes(b64) {
    const bin = _atob(String(b64).replace(/-/g, '+').replace(/_/g, '/'))
...
function unpackRouPng(bytes) {
    let n = 8
    while (n + 12 <= bytes.length) {
        const ln = ((bytes[n] << 24) >>> 0) | (bytes[n + 1] << 16) | (bytes[n + 2] << 8) | bytes[n + 3]
        const typ = String.fromCharCode(bytes[n + 4], bytes[n + 5], bytes[n + 6], bytes[n + 7])
        const dataStart = n + 8
        if (dataStart + ln > bytes.length) break
        if (typ === 'roUd') {
            if (ln < 1) return null
            const flag = bytes[dataStart]
            let payload = bytes.subarray(dataStart + 1, dataStart + ln)
            console.log('roUd flag=' + flag + ' len=' + payload.length)
            if (flag & 1) {
                payload = tryInflate(payload)
                console.log('after inflate len=' + (payload ? payload.length : 0))
            }
            return payload
        }
        n = dataStart + ln + 4
...
function tryInflate(data) {
    try {
        const p = (typeof __pako !== 'undefined' && __pako) ? __pako
            : (typeof globalThis !== 'undefined' && globalThis.__pako) ? globalThis.__pako
            : null
        if (p && p.inflate) {
            const out = p.inflate(data)
            return out instanceof Uint8Array ? out : new Uint8Array(out)
        }
    } catch (e) {
        console.log('inflate err: ' + e)
    }
    try {
    ...
    console.log('FATAL: no zlib available')
    return data
}
...
function rewriteM3u8(content, base) {
    ...
    for (const line of lines) {
        const s = line.trim()
        if (s && s[0] !== '#') {
            out.push(toProxy(joinUrl(base, s)))
        } else if (s.indexOf('URI="') >= 0) {
            out.push(line.replace(/URI="([^"]+)"/g, (_, u) => {
                return 'URI="' + toProxy(joinUrl(base, u)) + '"'
            }))
        } else {
            out.push(line)
        }
...
async function init(cfg) {
    console.log('rou init pako=' + (typeof __pako !== 'undefined'))
...
async function homeVod() {
    try {
        const html = await fetchHtml(HOST + '/home')
...
async function category(tid, pg, filter, extend) {
    try {
        ...
        const id = tid || '日本'
        const url = HOST + '/t/' + encodeURIComponent(id) + '?order=createdAt&page=' + page
        console.log('category: ' + url)
        const html = await fetchHtml(url)
        const list = parseList(html)
        console.log('category count: ' + list.length)
...
async function detail(ids) {
    try {
        ...
        id = String(id || '')
        if (id.indexOf('/v/') >= 0) id = id.split('/v/')[1].split(/[?#]/)[0]
        id = id.replace(/\/$/, '')
        const url = HOST + '/v/' + id
...
async function play(flag, id, vipFlags) {
    try {
        ...
        // 所有 /api/hls/ 和 .png 分片都必须走 proxy
        if (/\/api\/hls\//i.test(url) || /\.png(\?|$)/i.test(url) || /koct98|mekz27|rn252/i.test(url)) {
...
async function search(wd, quick, pg) {
    try {
        ...
        const page = parseInt(pg) || 1
        const url = HOST + '/search?keyword=' + encodeURIComponent(wd) + '&page=' + page
        const html = await fetchHtml(url)
        const list = parseList(html)
...
async function proxy(params) {
    try {
        ...
        console.log('proxy in: ' + raw.slice(0, 140))

        const buf = await fetchBin(raw)
        if (!buf || !buf.length) {
            console.log('proxy empty body')
            return [502, 'text/plain', 'empty body']
        }
        console.log('proxy body len=' + buf.length + ' magic=' + buf[0].toString(16))

        let body = buf
        if (isPng(buf)) {
            const payload = unpackRouPng(buf)
            if (!payload) return [502, 'text/plain', 'no roUd']
            body = payload
        }

        // m3u8?
        let head = ''
        const n = Math.min(16, body.length)
        for (let i = 0; i < n; i++) head += String.fromCharCode(body[i])
        if (head.indexOf('#EXT') === 0 || (body[0] === 0x23 && body[1] === 0x45)) {
            const text = utf8(body)
            const base = raw.replace(/[^/]*$/, '')
            const m3u8 = rewriteM3u8(text, base)
            console.log('proxy return m3u8 lines=' + m3u8.split('\n').length)
            return [200, 'application/vnd.apple.mpegurl', m3u8]
        }

        // TS 分片：返回 base64 + flag 1（兼容部分壳）
        // T4 常见：第三项直接二进制字符串 / Uint8Array
        console.log('proxy return ts len=' + body.length)
        try {
            return [200, 'video/MP2T', body]
        } catch (e) {
            return [200, 'video/MP2T', bytesToB64(body), {}, 1]
        }
    } catch (e) {
